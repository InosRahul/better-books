...
import { getError, Store, useStyles } from '../../utils';
...
function AdminDashboard() {
  ...
  const createHandler = async () => {
    ...
    try {
      ...
      router.push(`/admin/book/${data.product._id}`);
    } catch (err) {
  ...
  const deleteHandler = async productId => {
    ...
    try {
      ...
      await axios.delete(`/api/admin/books/${productId}`, {
        headers: { authorization: `Bearer ${userInfo.token}` },
      });
        ...
        <Grid item md={3} xs={12}>
          <Card className={classes.section}>
            <List>
              ...
              <NextLink href="/admin/books" passHref>
                      ...
                      <TableBody>
                        {books.map(book => (
                          <TableRow key={book._id}>
                            <TableCell>{book._id.substring(20, 24)}</TableCell>
                            <TableCell>{book.name}</TableCell>
                            <TableCell>₹{book.price}</TableCell>
                            <TableCell>{book.genre[0]}</TableCell>
                            <TableCell>{book.countInStock}</TableCell>
